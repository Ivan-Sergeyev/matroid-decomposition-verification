...
def BinaryMatroidStandardRepr.Regular {α : Type*} [DecidableEq α] (M : BinaryMatroidStandardRepr α) : Prop :=
  M.B.Regular
...
lemma StandardRepresentation.isRegularIff0 {α : Type*} [DecidableEq α] (M : BinaryMatroidStandardRepr α) :
    ...
    := by
  simp [BinaryMatroidStandardRepr.Regular, Matrix.Regular, Matrix.TU_adjoin_id_left_iff]

/-- The following statements are parts of Theorem (9.2.9) from Truemper's book. -/
theorem BinaryMatroidStandardRepr.isRegularIff1 {α : Type*} [DecidableEq α] (M : BinaryMatroidStandardRepr α) :
    M.Regular ↔ ∃ A : Matrix M.X M.Y ℚ, A.TU ∧ M.matroid.RepresentedBy A := by
  sorry

theorem BinaryMatroidStandardRepr.isRegularIff2 {α : Type*} [DecidableEq α] (M : BinaryMatroidStandardRepr α) :
    M.Regular ↔ ∀ F : Type*, ∃ hF : Field F, M.matroid.RepresentableOver F := by -- todo: check correctness of Field F
  sorry

theorem BinaryMatroidStandardRepr.isRegularIff3 {α : Type*} [DecidableEq α] (M : BinaryMatroidStandardRepr α) :
    M.Regular ↔ M.matroid.RepresentableOver Z2 ∧ M.matroid.RepresentableOver (ZMod 3) := by -- todo: check correctness of Field F
  sorry

theorem BinaryMatroidStandardRepr.isRegularIff4 {α : Type*} [DecidableEq α] (M : BinaryMatroidStandardRepr α) :
...
lemma BinaryMatroidStandardRepr_toMatroid_isRegular_iff {α : Type*} [DecidableEq α] {M₁ M₂ : BinaryMatroidStandardRepr α}
