...
structure VectorMatroid (α F : Type*) [Field F] where
...
def Matrix.VectorMatroid {α F : Type*} [Field F] {X Y : Set α} (A : Matrix X Y F) : VectorMatroid α F :=
  ⟨X, Y, A⟩

/-- todo: remove after 2-sum is fixed -/
def Matrix.IndepCols {α F : Type*} [Field F] {X Y : Set α} (A : Matrix X Y F) (S : Set α) : Prop :=
...
def VectorMatroid.IndepCols {α F : Type*} [Field F] (M : VectorMatroid α F) (S : Set α) : Prop :=
  ∃ hS : S ⊆ M.Y, LinearIndependent F (M.A.submatrix id hS.elem).transpose

/-- Empty set is independent. -/
theorem VectorMatroid.IndepCols_empty {α F : Type*} [Field F] (M : VectorMatroid α F) : M.IndepCols ∅ := by
  use Set.empty_subset M.Y
  exact linearIndependent_empty_type

/-- A subset of a linearly independent set of columns is linearly independent. -/
theorem VectorMatroid.IndepCols_subset {α F : Type*} [Field F] (M : VectorMatroid α F)
...
theorem VectorMatroid.IndepCols_aug {α F : Type*} [Field F] (M : VectorMatroid α F) (I J : Set α)
...
theorem VectorMatroid.IndepCols_maximal {α F : Type*} [Field F] (M : VectorMatroid α F) (S : Set α) :
    Matroid.ExistsMaximalSubsetProperty M.IndepCols S := by
  sorry

/-- Vector matroid expressed as `IndepMatroid`. -/
def VectorMatroid.IndepMatroid {α F : Type*} [Field F] (M : VectorMatroid α F) : IndepMatroid α where
  E := M.Y
  Indep := M.IndepCols
...
def VectorMatroid.matroid {α F : Type*} [Field F] (M : VectorMatroid α F) : Matroid α :=
  M.IndepMatroid.matroid


section Representability

def Matroid.RepresentedBy {α F : Type*} {X Y : Set α} [Field F] (M : Matroid α) (A : Matrix X Y F) : Prop :=
  M = (⟨X, Y, A⟩ : VectorMatroid α F).IndepMatroid.matroid

def Matroid.RepresentableOver {α : Type*} (M : Matroid α) (F : Type*) [Field F] : Prop :=
  ∃ M' : VectorMatroid α F, M'.IndepMatroid.matroid = M
...
@[simp]
lemma VectorMatroid.E_eq {α F : Type*} [Field F]
    (M : VectorMatroid α F) : M.matroid.E = M.Y := by rfl

@[simp]
lemma VectorMatroid.indep_eq {α F : Type*} [Field F]
  (M : VectorMatroid α F) : M.matroid.Indep = M.IndepCols := rfl
