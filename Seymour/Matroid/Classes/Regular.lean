...
theorem Matroid.RepresentableTU_RepresentableAnyField {α X : Type*} {Y : Set α} {A : Matrix X Y ℚ}
    (M : Matroid α) (hM : M.RepresentedBy A) (hA : A.IsTotallyUnimodular) :
...
lemma StandardRepresentation.isRegularIff0 {α : Type*} [DecidableEq α] (M : BinaryMatroid α) :
    ...
    := by
  simp [BinaryMatroid.Regular, Matrix.Regular, Matrix.one_fromCols_isTotallyUnimodular_iff]
...
theorem BinaryMatroid.isRegularIff1 {α : Type*} [DecidableEq α] (M : BinaryMatroid α) :
    M.Regular ↔ ∃ A : Matrix M.X M.Y ℚ, A.IsTotallyUnimodular ∧ M.matroid.RepresentedBy A := by
  sorry
...
theorem BinaryMatroid.isRegularIff4 {α : Type*} [DecidableEq α] (M : BinaryMatroid α) :
    ...
    (∀ A : Matrix M.X M.Y (ZMod 3), M.matroid.RepresentedBy A → A.IsTotallyUnimodular) := by -- todo: check correctness of Field F
  sorry
