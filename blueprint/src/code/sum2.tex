...
\begin{lemma}[$2$-sum of TU matrices is a TU matrix]
  \label{lem:code_2_sum_of_TU}
  \uses{def:code_2_sum_of_matrices,def:code_tu_matrix}
...
\begin{proof}[Proof sketch]
  \uses{def:code_2_sum_of_matrices,def:code_tu_matrix,lem:code_tu_add_ok_rows,cor:code_tu_add_ok_cols,lem:code_mvm_pivot,lem:code_mvm_props}
  % \leanok
...
\begin{lemma}[$2$-sum of regular matroids is a regular matroid]
...
\begin{proof}[Proof sketch]
  \uses{def:code_2_sum_of_binary,def:code_regular_matroid,lem:code_2_sum_of_TU}
  \leanok
  Let $B$, $B_{1}$, and $B_{2}$ be the representation matrices of $M$, $M_{1}$, and $M_{2}$, respectively.
  Apply Lemma~\ref{lem:code_2_sum_of_TU}.
\end{proof}
